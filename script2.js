...
let manoCompu = document.querySelector(".mano_computador");

let puntajeUsuario = document.querySelector(".puntaje_usuario p");
let puntajeComputadora = document.querySelector(".puntaje_computador p");
...
const swalGanador= () => {
  Swal.fire({
    icon: 'success',
    title: 'Has Ganaste esta partida!!',
    text: '¿Quieres otra ronda?',
    confirmButtonText: 'Lest Go!!',
    allowOutsideClick: false
  })
}
...
const resultado = () => {
  if (eleccionUsuario == "piedra") {
    ...
    if (eleccionCompu == "tijera") {
      labelResultado.textContent = "¡¡Has gano un punto!!";
      contUsuario++;
      puntajeUsuario.textContent = contUsuario;
      ganador(contUsuario, contCompu);
    }
